...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    daily_ret.index = daily_ret.index.to_period('M')
    vol_dict = {}

    for period, data in daily_ret.groupby(daily_ret.index):
        if len(data) >= 18:
            vol_dict[period] = data.std()
        else:
            vol_dict[period] = pd.Series([None] * data.shape[1], index=data.columns)

    vol_df = pd.DataFrame(vol_dict).T
    vol_df.columns = [f"{col}_{cha_name}" for col in vol_df.columns]
    vol_df = vol_df.astype('float64')
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    df_vol = df_vol.reindex(monthly_returns.index)
    df = merge_tables(ret, df_vol, cha_name)
