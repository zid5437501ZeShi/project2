...
def monthly_return_cal(prc):
    ...
    prc_monthly_return.index = prc_monthly_return.index.to_period('M')
    valid_months = prc.resample('M').count() >= 18
    valid_months = valid_months[valid_months].index.to_period('M')
    prc_monthly_return = prc_monthly_return[prc_monthly_return.index.isin(valid_months)]
    prc_monthly_return.index.name = 'Year_Month'
    prc_monthly_return.name = prc.name

    return prc_monthly_return
...
def aj_ret_dict(tickers, start, end):
    ...
    for ticker in tickers:
        ...
        daily_returns[ticker_lower] = daily_ret
        monthly_returns[ticker_lower] = monthly_ret
    ...
    values = {'Daily': daily_return_df, 'Monthly': monthly_return_df}
